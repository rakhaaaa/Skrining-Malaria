...
export default function SplashScreen() {
  ...
  const goToProtectedScreen = async (screen) => {
    // Jika token sesi masih ada, pengguna boleh langsung ke halaman tujuan.
    // Jika tidak ada, arahkan dulu ke halaman login.
    const session = await getSession();
    navigation.replace(session?.token ? screen : "Login");
  };

  return (
    // View ini menjadi layar pembuka pertama saat aplikasi dijalankan.
    <View style={styles.container}>
      <StatusBar barStyle="light-content" backgroundColor="#0A0F1E" />
      {/* Isi splash screen dibungkus animasi agar tampil lebih halus. */}
      <Animated.View style={[styles.content, { opacity: fadeAnim, transform: [{ translateY: slideAnim }] }]}>
        {/* Logo utama aplikasi ditampilkan di bagian tengah atas. */}
        <View style={styles.iconWrapper}>
          <Image source={require("../../assets/malaria.png")} style={styles.iconImg} />
        </View>
        {/* Judul dan subjudul menjelaskan fungsi utama aplikasi. */}
        ...
        {/* Bagian pill dipakai untuk menampilkan identitas pembuat aplikasi. */}
        <View style={styles.pills}>
          <View style={[styles.pill, styles.pillGreen]}><Text style={styles.pillTextGreen}>Rakha Naufal Sujana - 535220006</Text></View>
        </View>
        {/* Tombol utama membawa pengguna masuk ke halaman utama jika sudah login. */}
...
const styles = StyleSheet.create({
  ...
  // Gaya tombol utama dan tombol sekunder.
