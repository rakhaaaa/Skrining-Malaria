...
export default function FormScreen() {
  ...
  const clearError = (key) => {
    // Error per field dihapus saat pengguna mulai memperbaiki inputnya.
    setErrors(prev => {
  ...
  const handleSexChange = (value) => {
    // Pilihan jenis kelamin disimpan terpisah karena inputnya berbentuk tombol, bukan text field.
    setSex(value);
    clearError("sex");
  };

  const handleFieldChange = (key, value) => {
    // Semua input angka disimpan ke object form agar mudah dipetakan ke payload backend.
  ...
  const handleSubmit = async () => {
    // Submit hanya diproses jika validasi lolos agar backend tidak menerima data kosong/salah format.
    if (!validate()) {
    ...
    setLoading(true);
    try {
      // Payload backend mengikuti key yang sudah ditentukan di daftar FIELDS.
      const payload = { sex, ...Object.fromEntries(FIELDS.map(f => [f.key, form[f.key]])) };
      const res = await predictMalaria(payload);
      // Record lokal ini dipakai untuk menampilkan kembali seluruh data input bersama hasil prediksi.
      const record = {
  ...
  return (
    ...
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      >
        ...
        {/* Isi form dibuat scroll supaya semua input tetap bisa dijangkau di perangkat dengan layar kecil. */}
        <ScrollView
          style={styles.body}
        ...
        >
          <View style={styles.infoCard}>
            ...
            <Text style={styles.infoText}>Masukkan data pasien dan hasil pemeriksaan darah lengkap.</Text>
          </View>
          ...
          {/* Usia dipisah lebih dulu karena termasuk data dasar pasien, bukan parameter lab. */}
          <FieldRow field={FIELDS[0]} form={form} onChange={handleFieldChange} errors={errors} />

          {/* Kelompok hematologi memuat parameter darah utama yang dipakai model. */}
          ...
          {/* Kelompok eritrosit menampilkan indeks sel darah merah yang ikut dianalisis model. */}
          <Text style={styles.sectionTitle}>Indeks Eritrosit</Text>
          {FIELDS.filter(f => f.group === "eritrosit").map(f => (
            <FieldRow key={f.key} field={f} form={form} onChange={handleFieldChange} errors={errors} />
          ))}

          {/* Kelompok trombosit dipisah agar struktur form lebih mudah diikuti pengguna. */}
...
function FieldRow({ field, form, onChange, errors }) {
  return (
    // Komponen ini dipakai berulang untuk menjaga tampilan setiap field numerik tetap konsisten.
    <View style={styles.fieldWrap}>
